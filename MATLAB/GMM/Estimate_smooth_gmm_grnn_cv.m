function [opp, ocv] = Estimate_smooth_gmm_grnn_cv(ipp, frozen)
%function [opp, ocv] = Estimate_smooth_gmm_grnn_cv(ipp, frozen)
...
% by the leave-one-event-out cross-validation (LOEOCV)
% Inputs:
%  ipp - input parameters
%     (see compute_gmm_grnn_cv)
...
%
    ...
    [osigmas,ocv] = fminsearch(@(sigmas)compute_gmm_grnn_cv_local(opp, sigmas, frozen), initsigma, options) ;
    opp.GRNN_smoothing_parameters(~frozen) = 10.0 .^ osigmas;

end
function ocv = compute_gmm_grnn_cv_local(pp, sigmas, frozen)
